...
public class NodeRenderer<C> {
    ...
    private final expression.common.Renderer<C, Settings, expression.common.Node<C>> nodeRenderer = new Renderer<>(expression.common.Node::constant);
    ...
    public void unary(final String name) {
        nodeRenderer.unary(name, (settings, arg) -> {
            final expression.common.Node<C> inner;
            if (settings.mode == Mode.FULL) {
                ...
                inner = op.isEmpty() ? arg : expression.common.Node.op(op, arg);
            }
            return settings.extra(expression.common.Node.op(name, inner), random);
        });
    }
    ...
    private expression.common.Node<C> process(final Settings settings, final Priority mp, final expression.common.Node<C> l, final expression.common.Node<C> r) {
        if (settings.mode == Mode.FULL) {
        ...
        final expression.common.Node<C> al = paren(lp.compareLevels(mp) < 0, l);
        if (rc == 0 && !advanced) {
    ...
    private boolean hasOther(final expression.common.Node<C> node, final Priority priority) {
    ...
    private expression.common.Node<C> op(final Priority mp, final expression.common.Node<C> l, final expression.common.Node<C> r) {
        return mp.op(l, r);
    }

    private Priority priority(final expression.common.Node<C> node) {
        return get(node, () -> Priority.MAX, (n, a, b) -> Functional.get(priorities, n));
    }

    private <R> R get(final expression.common.Node<C> node, final Supplier<R> common, final expression.common.Node.Binary<expression.common.Node<C>, R> binary) {
        return node.get(c -> common.get(), n -> common.get(), (n, a) -> common.get(), binary);
    }

    public String render(final expression.common.Expr<C, ?> expr, final Settings settings) {
        return renderNode(renderToNode(settings, expr));
    }

    public String renderNode(final expression.common.Node<C> node) {
        return node.cata(
                String::valueOf,
                name -> name,
    ...
    public expression.common.Node<C> renderToNode(final Settings settings, final Expr<C, ?> expr) {
        return nodeRenderer.render(settings, expr.convert((name, variable) -> expression.common.Node.op(name)));
    }

    private static <C> expression.common.Node<C> paren(final boolean condition, final expression.common.Node<C> node) {
        return condition ? expression.common.Node.op(PAREN, node) : node;
    }
    ...
    private static final class Priority {
        ...
        public <C> expression.common.Node<C> op(final expression.common.Node<C> l, final expression.common.Node<C> r) {
            return expression.common.Node.op(op, l, r);
        }
    ...
    public static class Settings {
        ...
        public <C> expression.common.Node<C> extra(Node<C> node, final ExtendedRandom random) {
