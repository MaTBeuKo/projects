...
public class Generator<C> {
    ...
    public void add(final String name, final int arity) {
        ops.add(expression.common.Op.of(name, arity));
        forbidden.add(name);
    }

    private expression.common.Node<C> generate(
            final List<expression.common.Node<C>> variables,
            final boolean nullary,
            final Supplier<expression.common.Node<C>> unary,
            final Supplier<Pair<expression.common.Node<C>, expression.common.Node<C>>> binary
    ) {
        if (nullary || ops.isEmpty()) {
            return random.nextBoolean() ? random.randomItem(variables) : expression.common.Node.constant(constant.get());
        } else {
            final Op<Integer> op = random.randomItem(ops);
            if (op.value == 1) {
                return expression.common.Node.op(op.name, unary.get());
            } else {
                final Pair<expression.common.Node<C>, expression.common.Node<C>> pair = binary.get();
                return expression.common.Node.op(op.name, pair.first(), pair.second());
            }
        }
    }

    private expression.common.Node<C> generate(final List<expression.common.Node<C>> variables, final boolean nullary, final Supplier<expression.common.Node<C>> child) {
        return generate(variables, nullary, child, () -> Pair.of(child.get(), child.get()));
    }

    private expression.common.Node<C> generateFullDepth(final List<expression.common.Node<C>> variables, final int depth) {
        return generate(variables, depth == 0, () -> generateFullDepth(variables, depth - 1));
    }

    private expression.common.Node<C> generatePartialDepth(final List<expression.common.Node<C>> variables, final int depth) {
        return generate(variables, depth == 0, () -> generatePartialDepth(variables, random.nextInt(depth)));
    }

    private expression.common.Node<C> generateSize(final List<expression.common.Node<C>> variables, final int size) {
        return generate(
                variables,
                size == 0,
    ...
    public <E> void testRandom(
            ...
            final Consumer<expression.common.Expr<C, E>> consumer
    ) {
    ...
    private <C, E> void testRandom(
            ...
            final int levels,
            final int perLevel,
            final BiFunction<List<expression.common.Node<C>>, Integer, expression.common.Node<C>> generator
    ) {
